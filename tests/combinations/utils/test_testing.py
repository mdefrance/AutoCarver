...
def test_minimum_frequency_all_above():
    ...
    frequencies = pd.Series([0.2, 0.3, 0.4, 0.5])
    min_freq = 0.1
...
def test_minimum_frequency_some_below():
    ...
    frequencies = pd.Series([0.2, 0.3, 0.05, 0.5])
    min_freq = 0.1
    result = _test_minimum_frequency_per_modality(frequencies, min_freq)
    assert result is False
...
def test_minimum_frequency_all_below():
    ...
    frequencies = pd.Series([0.05, 0.08, 0.09, 0.05])
    min_freq = 0.1
    result = _test_minimum_frequency_per_modality(frequencies, min_freq)
    assert result is False
...
def test_minimum_frequency_exactly_at_threshold():
    ...
    frequencies = pd.Series([0.1, 0.1, 0.1, 0.1])
    min_freq = 0.1
    result = _test_minimum_frequency_per_modality(frequencies, min_freq)
    assert result is True
...
def test_viability_empty_rates():
    ...
    rates = pd.DataFrame({"frequency": [], "target_mean": []})
    min_freq = 0.1
    result = _test_viability(rates, min_freq, "target_mean")
    expected = {
